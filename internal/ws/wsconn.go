...
func (c *webSocketConnection) dispatchAckOrError(t *webSocketTopic, msg *WebSocketCommandMessage, err error) bool {
	if err != nil {
		...
		log.L(c.ctx).Debugf("Received ack for batch %d on topic '%s': %s", msg.BatchNumber, t.topic, err)
	}
	select {
	case t.receiverChannel <- &WebSocketCommandMessageOrError{Msg: msg, Err: err}:
	default:
		log.L(c.ctx).Debugf("Spurious ack for batch %d on topic '%s' (closing connection): %s", msg.BatchNumber, t.topic, err)
